...
export default function AdminSidebar({ activeTab, sidebarOpen, onTabChange, onCloseSidebar, menuItems, role = 'admin', user }: AdminSidebarProps) {
  ...
  const defaultMenuItems: MenuItem[] = role === 'admin'
    ? [
        ...
        { id: 'manajemen', label: 'Manajemen', Icon: LayoutGrid, subItems: [
  ...
  const toggleExpand = (itemId: string) => setExpandedItems((prev) => prev.includes(itemId) ? prev.filter((id) => id !== itemId) : [...prev, itemId]);
  const activeParentIds = items.filter((item) => item.subItems?.some((sub) => sub.id === activeTab)).map((item) => item.id);
  ...
  return (
    <aside
      className={`${sidebarRoleClass} fixed left-0 top-14 z-40 flex h-[calc(100vh-56px)] w-64 flex-col overflow-hidden border-r border-slate-800 bg-slate-950 text-white shadow-xl transition-transform duration-300 sm:top-16 sm:h-[calc(100vh-64px)] sm:w-72 md:sticky md:left-0 md:top-16 md:z-30 md:translate-x-0 md:shadow-none ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
          ...
          return (
            <div key={item.id} className="readpoint-admin-menu-group">
              <button
                ...
                className={`readpoint-admin-menu-item flex w-full items-center justify-between gap-3 rounded-xl px-3 py-3 text-left text-sm font-black transition ${isActive || anySubItemActive ? 'bg-white text-slate-950 shadow-lg' : 'text-slate-300 hover:bg-white/10 hover:text-white'}`}
              >
                <span className="flex min-w-0 flex-1 items-center gap-3">
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-current/10"><Icon size={18} aria-hidden="true" /></span>
                  <span className="readpoint-admin-menu-label block min-w-0 flex-1 truncate">{item.label}</span>
                </span>
                {hasSubItems && <ChevronDown size={16} className={`shrink-0 transition-transform ${isExpanded ? 'rotate-180' : ''}`} aria-hidden="true" />}
              </button>
        ...
        {user ? (
          <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3">
            <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg bg-white/10 text-sm font-black text-white ring-1 ring-white/10">
              {user.profile_photo_url ? <img src={user.profile_photo_url} alt={user.name} className="h-full w-full object-cover" /> : <span>{user.name?.charAt(0).toUpperCase() || 'U'}</span>}
            </div>
            <div className="min-w-0 flex-1">
